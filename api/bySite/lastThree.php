...
if(isset($_GET['type'])){
  ...
  if($type == 'hours'){
    ...
    FROM time_entry t1 left outer join company on t1.company_RecID = company.company_RecID left outer join sr_service on t1.sr_service_recid = sr_service.sr_service_RecID where t1.pm_project_recid is null and company_name = '$company' and t1.date_start >= dateadd(mm,datediff(mm,0,getdate())-3,0)  and t1.date_start < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)   group by site_name";
  ...
  count(sr_service_recid)/3 as avg,
    ...
    where company_name = '$company' and parent is null and date_entered >= dateadd(mm,datediff(mm,0,getdate())-3,0)  and date_entered < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
